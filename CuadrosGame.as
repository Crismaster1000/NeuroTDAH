package {
	...
	public class CuadrosGame extends MovieClip {
		...
		var bosque: DESIERTO = new DESIERTO();
		var casa: INVIERNO = new INVIERNO();
		var jardin: OTOÑO = new OTOÑO();
		var naturaleza: PRIMAVERA = new PRIMAVERA();
		var sol: Verano = new Verano();
		...
		public function CuadrosGame() {
			...
			imagenes.push(bosque, casa, jardin, naturaleza, sol);
		...
		public function agregar_imagen(imagenes_array: Array) {
			...
			setTimeout(eliminar_cuadros, 9000, 3);
			...
			if (flag) {
				...
				if (fase > 4){
					// enviamos los datos
						...
						var datos: Array = [timer.time_execute, "1", df.format(new Date()), memoria_juegos.data.usuario.id, "3"];
						post_data.post_data(datos);

						MovieClip(this.root).gotoAndPlay(1, "PASASTE");
						//MovieClip(this.parent).gotoAndPlay(1, "NivelesSimon");
