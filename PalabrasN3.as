package {
	...
	public class PalabrasN3 extends MovieClip {

		var silabas_compuestas = ["PLOMERO", "PLACA", "ECLIPSE", "SOMBRERO", "BOLIGRAFO", "CUADRO", "PRECIO", "COCODRILO", "FRESA", "NEGRO", "SECRETO", "BICICLETA", "INCENDIO", "BLUSA", "PLANTA", "IGLESIA", "FLECHA", "FRASCO", "BLANCO", "PROMESA"];
		...
		public function inicio() {
			...
			var ran = randomRange(0, silabas_compuestas.length - 1);
			// asignamos una palabra
			palabra = silabas_compuestas[ran];
			// mostramos la palabra
			palabra_inicial.text = palabra;
			silabas_compuestas.removeAt(ran);
			...
			if (nump == palabra_array.length) {
				if (palabra_a == palabra_b) {
					...
					if (fase < 10) {
						...
						var datos: Array = [timer.time_execute, "3", df.format(new Date()), memoria_juegos.data.usuario.id, "2"];
		...
		public function addballs(palabra: Array) {
			...
			for (var i: int = 0; i < palabra.length; i++) {
				//instanciamos la Ticket
				var letrita: Ticket = new Ticket();
				...
				// agregamos la Ticket a la lista
		...
		function presionado(clickInfo: MouseEvent) {
			...
			// eliminamos el listener de la Ticket presionada
			this.removeEventListener(Event.ENTER_FRAME, movearray);
			// hacemos que la Ticket desaparezca
