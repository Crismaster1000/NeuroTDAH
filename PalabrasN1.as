package {
	...
	public class PalabrasN1 extends MovieClip {

		var silabas_simples = ["MAMA", "PAPA", "SEMAFORO", "LLAVE", "JIRAFA", "FUTURO", "RANITA", "CABEZA", "CABALLO", "CONEJO", "TOMATE", "MALETA", "GALLETA", "TESORO", "PIÑATA", "PAYASO", "PAJARO", "PILOTO", "NIÑO", "BIGOTE"];
		...
		public function inicio() {
			...
			var ran = randomRange(0, silabas_simples.length - 1);
			// asignamos una palabra
			palabra = silabas_simples[ran];
			// mostramos la palabra
			palabra_inicial.text = palabra;
			silabas_simples.removeAt(ran);
			...
			if (nump == palabra_array.length) {
				if (palabra_a == palabra_b) {
					...
					if (fase < 5) {
						...
						var datos: Array = [timer.time_execute, "1", df.format(new Date()), memoria_juegos.data.usuario.id, "2"];
		...
		public function movearray(event: Event): void {
			var letrita: Hoja = new Hoja();
		...
		public function presionado(clickInfo: MouseEvent) {
			//removeChild(letrita);
			...
			// eliminamos el listener de la Hoja presionada
			this.removeEventListener(Event.ENTER_FRAME, movearray);
			// hacemos que la Hoja desaparezca
		...
		public function repetir(clickInfo: MouseEvent) {
